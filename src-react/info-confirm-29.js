...
import Pay_qd from './component/pay/Pay29';
import WebViewLifeCycle from './lib/WebViewLifeCycle'
import PayDialog_qd from './component/pay/PayDialog29'
import TopTips from './component/TopTips/TopTips'
import JSBridge from './lib/JSBridge'
import './info-confirm-29.less'
import hybridAPI from './lib/hybridAPI'

export default class InfoConfrim29 extends SmartBlockComponent {
  constructor(props) {
    super(props);
  ...
  //渲染页面
  render(){
    const {scheduleId,selected, medAmPm,doctName,notRecord,showMask, medDate, patientIdType,regAmount,patientIdNo, deptName, corpName, corpId, needSource, patientId, patientName, discountFee, regMode, submit, outId, expirationTime,needDefault,patientCardCount} = this.state;
    var ampm = {"1": "上午", "2": "下午"};
    ...
    return (
      <div className="info-29_container">
         {/* 遮罩 */}
        ...
        <div style={{overflowY:"scroll",position:"relative", height: "100%",paddingBottom:'60px'}} id="listItems">
          ...
          {/* 预约操作,默认展示的就诊人必须绑卡,或者没有建立档,否则不展示;挂号操作,默认展示的就诊人必须建档 */}
            {regMode == 1&&(patientCardCount > 0 || notRecord == true) ?
            ...
            {regMode == 2&&patientCardCount > 0 ?
              (<li id="infoShow" className="txt-arrowlink">
                <span>{patientName}</span>
                {patientIdType == 1? //成人
                  <span><i>身份证</i><i style={{color:"#666",marginLeft:'10px'}}>{hideIdNo}</i></span>
                    :
                  <span><i>监证</i><i style={{color:"#666",marginLeft:'10px'}}>{hideIdNo}</i></span>
              } 
            </li>):null}
