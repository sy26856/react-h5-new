 ...
 */
export default class Pay extends SmartComponent {
  /**
   props = {
      corpId: //医院id
  ...
  initData(nextId) {
    //获取支持的支付方式
    let {corpId, optType, patientId,regType} = this.props;
    //获取当前支持的支付方式
    UserCenter.getPayTypes(corpId, optType, nextId || patientId, regType).subscribe(this).fetch();
  }
  ...
  onSuccess(result) {
    ...
    this.props.getBalance ? this.props.getBalance(accoutPay.balance):''
    let payTypes = [];
  ...
  render() {
    ...
    let {patientId, corpId, redirect, price, optType,regType} = this.props;
    price = price || 0;
    //是否至少有一种支付方式可用
        ...
        }
        {
          isAccoutPay ? <label htmlFor="radio1" className="list-label-wrapper">
              <div className="list-item list-item-middel list-nowrap">
                <div className="list-icon icon-online"></div>
                <div className="list-content ">
                  <div className="list-title ">{accoutPay.name} <span className="pay-number">({accoutPay.cardNo.slice(-4)})</span></div>
                  <div className=" list-brief txt-insign ">卡内余额:
                    <span className="txt-highlight">
                        <span className="txt-size-sm">￥</span>
                        {util.rmb(accoutPay.balance / 100)}
                      </span>
                  </div>
                  {
                  optType == 6 ? (regType == 6 ? <div className="list-brief">一卡通行，额度共用</div>:<div className="list-brief">就诊当天直接取号，节省等待时间</div>) : null
                  }
                </div>
            ...
            <label htmlFor="radio2" className="list-label-wrapper">
              <div className="list-item list-item-middel list-nowrap">
                ...
                <div className="list-content ">
                  ...
                  <div className="list-brief txt-nowrap">预约费用在取号时收取</div>
                </div>
                <label className="radio-wrapper radio-right">
